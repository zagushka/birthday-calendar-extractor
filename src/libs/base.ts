...
export abstract class CalendarBase<GeneratedEvent, GeneratedCalendar> {
  ...
  save(calendarData: string, subscriber?: Subscriber<any>) {
    const url = URL.createObjectURL(new Blob([calendarData], { endings: "transparent", type: this.fileMimeType }));
    chrome.downloads.download({ url, filename: this.filename }, (downloadId) => {
      if (subscriber) {
